...
public class FlowerWorldGen implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		...
		if (world.provider.getDimensionId() == 0 && !TragicConfig.getBoolean("allowExtraOverworldFlowers") || world.provider.getDimensionId() != 0 && !(world.provider instanceof TragicWorldProvider) || !TragicConfig.getBoolean("allowFlowerGen")) return;
		...
		if (world.provider.getDimensionId() == 0) //discriminator based flower generation for the overworld
		{
			...
			for (int i = 0; i < biome.theBiomeDecorator.flowersPerChunk * 4 && discrim[meta]; i++)
			{
				...
				if (world.isAirBlock(new BlockPos(Xcoord, Ycoord, Zcoord)) && (!world.provider.getHasNoSky() || Ycoord < 255) && flower.canBlockStay(world, new BlockPos(Xcoord, Ycoord, Zcoord), flower.getStateFromMeta(meta)))
				{
