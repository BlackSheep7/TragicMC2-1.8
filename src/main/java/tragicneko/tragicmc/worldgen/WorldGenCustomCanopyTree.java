...
public class WorldGenCustomCanopyTree extends WorldGenAbstractTree
{
	private final IBlockState logState;
	private final IBlockState leafState;
	
	public WorldGenCustomCanopyTree(IBlockState log, IBlockState leaf)
	{
		super(false);
		this.logState = log;
		this.leafState = leaf;
...
/*//TODO fix vines generating on dark trees
	private void generateVines(World world, BlockPos pos, int meta)
	{
		...
		while (true)
		{
			...
			if (!(world.getBlock(pos).isAir(world, pos)) || i1 <= 0)
			{
				return;
			}
