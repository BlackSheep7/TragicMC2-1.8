		...
		else
		{
			...
			if (!par2World.isRemote)
			{
				...
				meow: for (double d = 0.0D; d <= 30.0; d += 0.25D)
				{
					...
					List<BlockPos> list2 = WorldHelper.getBlocksInSphericalRange(par2World, 0.45, vec31.xCoord, vec31.yCoord + par3EntityPlayer.getEyeHeight(), vec31.zCoord);
					Block block;
					AxisAlignedBB bb2;

					for (BlockPos coords : list2)
					{
						block = par2World.getBlockState(coords).getBlock();

						bb2 = block.getCollisionBoundingBox(par2World, coords, par2World.getBlockState(coords));
