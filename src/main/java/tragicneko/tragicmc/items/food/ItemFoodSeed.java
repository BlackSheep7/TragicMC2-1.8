...
public class ItemFoodSeed extends Item {
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		else if (worldIn.canBlockBePlaced(this.block, pos, false, side, (Entity)null, stack))
		{
			...
			if ((this.block instanceof BlockFruit) && ((BlockFruit) this.block).canBlockStay(worldIn, pos, iblockstate1))
			{
				if (block instanceof BlockGenericLog)
				{
					...
					{
						iblockstate1 = this.block.onBlockPlaced(worldIn, pos, side, hitX, hitY, hitZ, 0, playerIn);
