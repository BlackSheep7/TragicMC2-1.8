	...
	private float[][] formSizes = new float[][] {{1.375F, 2.575F}, {0.725F, 2.575F}, {1.385F, 3.325F}, {1.7835F, 5.15F}, {1.135F, 1.575F}, {0.825F, 0.725F}, {1.335F, 2.675F},
		...
		public boolean attackEntityFrom(DamageSource source, float damage)
		{
			...
			if (this.getEntityForm() != 0)
			{
				...
				if (!source.isDamageAbsolute() && !source.canHarmInCreative() && !source.isUnblockable()) damage -= this.getTotalArmorValue();
				if (this.getHealth() - MathHelper.clamp_float(damage, 0F, (float) TragicConfig.getInt("bossDamageCap")) <= 0F)
				{
			...
			boolean flag = super.attackEntityFrom(source, damage);
			if (flag && this.getHealth() <= 0 && this.getEntityForm() != 0)
			{
				this.setEntityForm(0);
			}
