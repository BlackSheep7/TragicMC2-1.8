...
public class TragicConfig {
	...
	public static void load(Configuration config)
	{
		...
		prop = config.get(cat.getName(), s, 15);
		prop.comment = "The overall structure rarity.";
	...
	private static void postProcessConfigs()
	{
		...
		if (!mobConfig[15]) //ridable entities
		{
			for (b = 0; b < ridableAllow.length; b++) ridableAllow[b] = false;
		}
	...
	private static void registerArrayedVariables()
	{
		...
		registerObject("allowRidableEntities", mobConfig[++m]);
		registerObject("allowRidableEntityAbilities", mobConfig[++m]);
		registerObject("allowRidableEntityAbilitiesViaMob", mobConfig[++m]);
		...
		registerObject("allowJetNeko", mobAllow[++m]);
		registerObject("allowScienceNeko", mobAllow[++m]);
		registerObject("allowMechaNeko", mobAllow[++m]);
		registerObject("allowAssaultNeko", mobAllow[++m]);
		registerObject("allowCommonNeko", mobAllow[++m]);
		registerObject("allowTraderNeko", mobAllow[++m]);
		
		registerObject("allowMechaExo", ridableAllow[m = 0]);
		...
		registerObject("allowProfessorNekoid", bossAllow[++m]);
